...
else
{
	...
	/*
	FILE *fff = fopen ("/home/hiryugan/Documents/torcs/torcs-server/torcs-1.3.7-patched/src/drivers/scr_server/we.txt","a");
	fprintf(fff,"driver-%f,%f,%f,%f,%f,%f\n",car->_curTime, car->_accelCmd, car->_brakeCmd, car->_gearCmd, car->_steerCmd, car->_clutchCmd);
	std::cout << "weeeee" << endl;
	fclose(fff);
	*/
